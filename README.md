...
The maximum socket path in Linux Kernel is `107` characters + `\0`:
...
The `--chroot-base` value must maximum length of `31` characters. The constant jailer path suffix used by `firebuild` is `76` characters:
...
Thus **using more than `31` characters for the `--chroot-base` value, regardless if in profile or using the command `--chroot-base` flag, will lead to a very obsure error**. Firecracker will report an error similar to:
