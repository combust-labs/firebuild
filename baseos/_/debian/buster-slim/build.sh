...
docker exec ${CONTAINER_ID} /bin/sh -c 'for d in bin dev etc lib lib64 root sbin usr var; do tar c "/$d" | tar x -C /export-rootfs; done; exit 0'
docker exec ${CONTAINER_ID} /bin/sh -c 'for dir in boot opt proc run srv sys tmp; do mkdir /export-rootfs/${dir}; done; exit 0'
